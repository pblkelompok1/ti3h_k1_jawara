...
class IuranOtomatisDetailPage extends StatefulWidget {
  final Map<String, dynamic> data;

  const IuranOtomatisDetailPage({super.key, required this.data});

  @override
  State<IuranOtomatisDetailPage> createState() =>
      _IuranOtomatisDetailPageState();
}

class _IuranOtomatisDetailPageState extends State<IuranOtomatisDetailPage> {
  ...
  void initState() {
    ...
    String? kategori = widget.data["category"];
    ...
    String? hari = widget.data["hari"];
    if (hari != null && _hariList.contains(hari)) {
      _selectedHari = hari;
    }

    int? minggu = widget.data["minggu"];
    if (minggu != null && minggu >= 1 && minggu <= 4) {
  ...
  void _delete() {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text("Iuran otomatis berhasil dihapus.")),
    );
  }
